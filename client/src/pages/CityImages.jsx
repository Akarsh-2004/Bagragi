...
const CityImages = () => {
  ...
  useEffect(() => {
    const fetchImages = async () => {
      ...
      try {
        ...
        // Use the same endpoint structure as in Home component
        const res = await fetch(`http://localhost:5000/api/pexels/images?query=${encodeURIComponent(decodedCity)}`);
        ...
        setImages([]);
        setError(`Failed to fetch images for ${decodedCity}. Please check your connection and try again.`);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-100 to-indigo-200 flex flex-col p-6">
          ...
          <div className="w-full max-w-6xl">
            ...
            <div className="relative w-full h-[600px] overflow-hidden rounded-xl shadow-2xl bg-white">
              <img
                ...
                alt={images[current]?.description || `Scene from ${decodedCity}`}
                className="w-full h-full object-cover transition-opacity duration-1000"
                onError={(e) => {
                  console.error('Image failed to load:', images[current]?.url);
                }}
              />
                ...
                <div className="flex justify-between items-end">
                  <div className="text-white">
                    <p className="text-lg font-medium mb-2">
                      {images[current]?.description || `A beautiful view of ${decodedCity}`}
                    </p>
